...
export function AdminDashboard({ currentRole = 'Hotel Manager' }: AdminDashboardProps) {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="bg-gradient-to-br from-[#001f3f] to-[#003366] text-white rounded-2xl p-5 shadow-lg space-y-3 border border-[#D4AF37]/35 transform hover:-translate-y-1 transition-transform">
              ...
              <div className="space-y-1">
                <h3 className="text-2xl font-bold font-mono text-[#F9D976]">₹{metrics.totalRevenue.toLocaleString('en-IN')}</h3>
                <p className="text-[10px] text-slate-300 font-medium">GST Taxes (12-18%): ₹{metrics.gstCollected.toLocaleString('en-IN')}</p>
              </div>
            </div>
            ...
            <div className="bg-white text-slate-900 rounded-2xl p-5 border border-[#D4AF37]/20 border-l-4 border-l-[#003366] shadow-sm space-y-3 hover:shadow-md transition-shadow">
              ...
              <div className="space-y-1">
                <div className="flex items-baseline gap-2">
                  <h3 className="text-2xl font-bold font-mono text-[#003366]">{metrics.occupancyRate}%</h3>
                  <span className="text-xs text-slate-500">({metrics.occupiedRooms} / 9 Rooms occupied)</span>
                </div>
                {/* Visual Progress rate */}
                <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                  <div className="h-full bg-emerald-500 transition-all duration-500" style={{ width: `${metrics.occupancyRate}%` }} />
                </div>
              </div>
            </div>
            ...
            <div className="bg-white text-slate-900 rounded-2xl p-5 border border-[#D4AF37]/20 border-l-4 border-l-[#003366] shadow-sm space-y-3">
              ...
              <div className="space-y-1">
                <h3 className="text-2xl font-bold font-mono text-[#003366]">{metrics.totalBookings} Registered</h3>
            ...
            <div className="bg-white text-slate-900 rounded-2xl p-5 border border-[#D4AF37]/20 border-l-4 border-l-[#003366] shadow-sm space-y-3">
              ...
              <div className="space-y-1">
                <h3 className="text-2xl font-bold font-mono text-[#003366]">{metrics.complaintResolutionRate}%</h3>
                <p className="text-[10px] text-slate-400">({metrics.averageRating} Stars average ratings from reviews)</p>
              </div>
            </div>
            ...
            <div className="lg:col-span-2 bg-white p-6 rounded-2xl border border-slate-200/80 shadow-md space-y-4">
              ...
              <div className="h-64 flex items-end justify-between gap-3 pt-6 border-b border-l border-slate-100 px-4 pb-2" id="svg_report_trend">
                {monthlyTrend.map((t, idx) => {
                  ...
                  const percentage = maxRevenue > 0 ? (t.revenue / maxRevenue) * 90 : 50;

                  return (
                    <div key={idx} className="flex-1 flex flex-col items-center gap-2 group relative">
                      {/* Hover Tooltip tooltip */}
                      <span className="absolute -top-8 bg-slate-950 text-white font-mono text-[9px] p-1 px-1.5 rounded opacity-0 group-hover:opacity-100 transition-opacity z-10 font-bold whitespace-nowrap">
                        ₹{t.revenue.toLocaleString('en-IN')}
                      </span>

                      {/* Animated SVG bar */}
                      <div 
                        className="w-full bg-gradient-to-t from-[#001f3f] to-[#003366] hover:from-[#D4AF37] hover:to-[#F9D976] rounded-t-md transition-all duration-700"
                        style={{ height: `${percentage}%` }}
                      />
                      
                      <span className="text-[10px] text-slate-500 font-bold font-mono uppercase">{t.name}</span>
                    </div>
                  );
                })}
              </div>
                ...
                {popularRooms.map((r, i) => (
                  <div key={i} className="space-y-1.5">
                    <div className="flex justify-between items-center text-xs">
                      ...
                      <span className="font-mono text-slate-500">{r.occupancy}% Occupancy</span>
                    </div>
                    ...
                    <span className="text-[10px] text-slate-400 block font-mono">Gross generated revenue: ₹{r.revenue.toLocaleString('en-IN')}</span>
                  </div>
                ))}
              </div>
            </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {corporate.map((c) => (
                  <tr key={c.corporate_booking_id} className="hover:bg-slate-50/50">
                    ...
                    <td className="p-4 text-slate-500 font-mono">{c.contact_phone}</td>
                    <td className="p-4 font-mono font-bold text-slate-700">{c.number_of_rooms} Rooms</td>
                    <td className="p-4 text-slate-500">{c.booking_dates}</td>
                    <td className="p-4">
