...
export function GuestAuthGate({ loggedInGuest, onLogin, onLogout, children }: GuestAuthGateProps) {
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    if (!username || !password) {
      setErrorMsg("Please fill in both Guest ID / Username and Password.");
      return;
    }
  ...
  if (!loggedInGuest) {
    return (
      <div className="max-w-4xl mx-auto py-12" id="private_access_login_container">
        
        {/* Prominent Access Restricted Warning Header */}
        <div className="mb-8 bg-amber-50 border border-amber-300 rounded-2xl p-5 shadow-sm text-center space-y-2">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-bold text-amber-800 bg-amber-100 uppercase tracking-widest animate-pulse">
            <ShieldAlert className="h-3.5 w-3.5 text-amber-600" />
            <span>Private Property Access Policy</span>
          </div>
          <h2 className="text-sm font-bold text-slate-900 tracking-tight">
            Only Approved Corporate & Individual Guests Can Access Booking Features
          </h2>
          <p className="text-xs text-slate-600 leading-relaxed max-w-2xl mx-auto">
            Guests without valid credentials cannot access: <strong>Room Availability, Pricing Information, Booking Forms, Guest Dashboard, or Hotel Services</strong>.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-stretch">
          
          {/* Authentic Corporate / Guest Credentials Form */}
          <div className="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-lg flex flex-col justify-between" id="card_guest_login_form">
            <div className="space-y-4">
              <div className="flex items-center gap-2 border-b pb-3 border-slate-100">
                <div className="w-8 h-8 rounded-lg bg-[#003366] text-white flex items-center justify-center">
                  <Lock className="h-4 w-4 text-[#F9D976]" />
                </div>
                <div>
                  <h3 className="text-sm font-bold text-slate-900 font-heading uppercase">Verified Guest Access</h3>
                  <p className="text-[10px] text-slate-500">Sign in to check rates & file reservation booking</p>
                </div>
              </div>
              ...
              <form onSubmit={handleLoginSubmit} className="space-y-4">
                <div>
                  <label className="text-[10px] font-bold text-slate-600 block mb-1 uppercase tracking-wider">Guest ID / Username</label>
            ...
            <div className="mt-8 border-t pt-4 text-[10px] text-slate-400 space-y-1 bg-slate-50 -mx-6 -mb-6 p-4 rounded-b-2xl">
              ...
              <p className="text-[9px] text-slate-400 leading-normal italic">Tip: You can dynamically formulate alternative guest logins within Front Desk Staff or Hotel Manager role views, then simulate login here.</p>
            </div>
          </div>

          {/* Secure Request Informational Banner */}
          <div className="bg-gradient-to-br from-[#001f3f] to-[#012d59] text-slate-300 rounded-2xl p-6 border border-[#D4AF37]/20 flex flex-col justify-between" id="card_how_to_acquire">
            <div className="space-y-4">
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[9px] font-bold text-[#F9D976] bg-amber-500/10 uppercase tracking-widest border border-[#D4AF37]/35">
              ...
              <h3 className="text-sm font-extrabold text-[#F9D976] font-heading uppercase tracking-wide">
                How to Acquire Guest Credentials?
              </h3>
              
              {/* Timeline list */}
              <div className="space-y-4 text-xs">
                <div className="flex gap-2.5 items-start">
                  <div className="w-5 h-5 rounded-full bg-[#D4AF37]/15 border border-[#D4AF37]/45 flex items-center justify-center text-[10px] font-bold text-[#F9D976] mt-0.5">1</div>
                  <p className="text-slate-200">Contact Sai Nirvana Plaza Reservations Desk directly via Phone, Email or corporate inquiry details shown adjacent.</p>
                </div>
                <div className="flex gap-2.5 items-start">
                  <div className="w-5 h-5 rounded-full bg-[#D4AF37]/15 border border-[#D4AF37]/45 flex items-center justify-center text-[10px] font-bold text-[#F9D976] mt-0.5">2</div>
                  <p className="text-slate-200">Our Desk staff verifies credentials and formalizes a unique, non-public Guest Access Voucher.</p>
                </div>
                <div className="flex gap-2.5 items-start">
                  <div className="w-5 h-5 rounded-full bg-[#D4AF37]/15 border border-[#D4AF37]/45 flex items-center justify-center text-[10px] font-bold text-[#F9D976] mt-0.5">3</div>
                  <p className="text-slate-200">Direct transmission to your registered contacts using secure <strong>WhatsApp Business APIs</strong> or official professional emails.</p>
                </div>
              </div>
            </div>

            <div className="border-t border-white/10 pt-4 space-y-2 mt-4 text-[11px] text-slate-200 uppercase tracking-wider font-semibold">
              <p className="flex items-center gap-2"><Phone className="h-3.5 w-3.5 text-[#F9D976]" /> +91 11-4560-6000 ext 201</p>
              <p className="flex items-center gap-2"><Mail className="h-3.5 w-3.5 text-[#F9D976]" /> desk@sri-nirvana-plaza.com</p>
            </div>
          </div>
